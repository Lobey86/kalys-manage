jQuery.fn.highlight = function(selector, className, eventStart, eventEnd) {
	...
	} else if (eventStart == eventEnd || eventStart != undefined
			&& eventEnd == undefined) {
		var toggle = true;
	}
	...
	this
			.each(function() {
				...
				if (tagName == 'form') {
					...
					var elements = jQuery(
							"textarea, select, multi-select, :text, :image, :password, :radio, :checkbox, :file",
							this);
					...
					if (toggle) {
						elements.bind(eventStart, function() {
						...
						elements.bind(eventEnd, function() {
							jQuery(this).removeClass(className);
						});
					}
